...
Playbook **`12_setup_freeipa_server.yml`** gates **`ipa-server-install`** on three checks—**`ipa --version`** (CLI usable / configured; **`ipa-server-install --version`** is logged for debug only and reflects installer RPM, not configured state; **`ipactl --version`** is not valid), **`ipactl status`**, and **`/etc/ipa/default.conf`**—summarized in one debug line (ipa CLI rc, ipactl rc, default.conf, VPC vs **`--no-forwarders`** mode). Install runs when any gate check fails or **`ipa_server_install_force: true`**; preflight directory creation runs only on that install path (**`ipa_server_etc_ipa_subdirs`** includes **`/etc/ipa/custodia`**). DNS forwarder argv comes from **`resolve_ipa_install_dns_forwarders.yml`** (**`ipa_install_dns_use_vpc_forwarder`** → **`--forwarder=<VPC>`**; **`ipa_install_dns_no_forwarders`** → **`--no-forwarders`**).
