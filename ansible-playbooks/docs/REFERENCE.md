...
Playbook **`12_setup_freeipa_server.yml`** gates **`ipa-server-install`** on three checks—**`ipa-server-install --version`**, **`ipactl status`**, and **`/etc/ipa/default.conf`**—summarized in one debug line (version rc, ipactl rc, default.conf, VPC vs **`--no-forwarders`** mode). Install runs when any check fails or **`ipa_server_install_force: true`**; preflight directory creation runs only on that install path. DNS forwarder argv comes from **`resolve_ipa_install_dns_forwarders.yml`** (**`ipa_install_dns_use_vpc_forwarder`** → **`--forwarder=<VPC>`**; **`ipa_install_dns_no_forwarders`** → **`--no-forwarders`**).
