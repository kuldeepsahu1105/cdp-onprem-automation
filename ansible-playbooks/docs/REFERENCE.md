...
| **06** / **23** / **31** psycopg2 | `psycopg2` is installed and verified in Ansible's target interpreter (`/usr/bin/python3` by default), the configured `python{{ python_version }}` interpreter, and before First Run in Hue's isolated parcel virtual environment when Hue is enabled; duplicate executable names are installed only once |
...
| `csd_respect_service_toggles` | `false` | Stage all configured CSDs; set true to include only CSDs for enabled `base_cluster_install_services` keys |
| `cm_repo_username` | — | Required (archive credentials) |
| `cm_repo_password` | — | Required (archive credentials) |

**CSD JAR override vs default:** Leave `scm_csds` and `scm_csds_redhat` empty (default) and set `cdv_version`, `cfm_version`, and jar basename vars — play **24** builds `scm_csds_effective` via `scm_csds_urls.j2` (DATAVIZ + NiFi + Registry by default) and downloads every URL directly to its exact filename under `/opt/cloudera/csd`. Download failures are fatal rather than falling back to unrelated parcel contents. Set `csd_respect_service_toggles: true` only when disabled optional services should also suppress their CSD downloads. To override all CSD URLs, set `scm_csds` to a YAML list of full `https://…/*.jar` strings (same shape the template emits). `scm_csds_effective` is computed in `set_os_facts.yml`: explicit list → else `scm_csds_redhat` → else `scm_csds_urls.j2` on RHEL when `csd_build_urls_enabled` is true.
...
|---|---|---|
| DATAVIZ CSD JAR `get_url` | `scm_csds_urls.j2` | `…/p/cdv/8.0.7/redhat8/yum/DATAVIZ-8.0.7-b50.p1.71299628.jar` |
...
| `monitoring_cadvisor_host_groups` | `[ecs-masters, ecs-workers]` | Inventory groups for standalone host cAdvisor; the ops host is covered separately by the monitoring Compose cAdvisor |
...
Playbook **`12_setup_freeipa_server.yml`** imports **`detect_ipa_server_install_state.yml`** and skips **`ipa-server-install`** when **`ipa_server_has_default_conf`** is true, **`ipactl`** does not report **IPA is not configured**, and there is no partial install debris (**`ipa_server_has_partial_state`**). **`ipa --version`** remains diagnostic only. A non-zero **`ipactl`** rc from stopped services alone does **not** trigger reinstall; **`ensure_ipa_kdc_services.yml`** runs after the play. A clean host installs directly without uninstall. Best-effort **`ipa-server-install --uninstall`** runs only for concrete stale/partial server markers or **`ipa_server_install_force: true`**. DNS forwarder argv comes from **`resolve_ipa_install_dns_forwarders.yml`**.
