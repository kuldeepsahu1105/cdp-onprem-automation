...
| `common_tasks/configure_ipa_httpd_behind_caddy.yml` | **Opt-in** (`deployment_portal_ipa_httpd_proxy_enabled`, default **false**): **`zz-ipa-caddy-proxy.conf`** (checksum + legacy **`<Location /ipa>`** guard), **`ipa-rewrite.conf`**, **`apachectl configtest`**, **`systemctl reload httpd`** (PORTAL sync or play **16** when flag **true**) |
