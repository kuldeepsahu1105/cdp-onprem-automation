...
**CM remote parcel repositories:** Playbooks **25**, **31**, and **33** call `configure_cm_parcel_repo_api.yml`, which sets `REMOTE_PARCEL_REPO_URLS` to the CDH parcel URL (`parcel_repo`), ECS PVC DS (`ecs_parcel_repo_url` from `ecs_pvc_ds_version`, e.g. `1.5.5-h3300`), Intel MKL (`cm_remote_parcel_repo_intel_mkl_url`), and CDV/CFM archive directories (from `scm_csd_parcel_repo_urls.j2` unless `cm_remote_parcel_csd_repo_urls` is set). Legacy wizard URLs (`archive.cloudera.com/cdh7/…`, `latest_supported`, SPARK3 parcel paths) are stripped. `cm_parcel_install_csd_repo_urls: false` prevents CM from re-adding CSD-derived spark/cdh6 URLs on scm-server restart.
