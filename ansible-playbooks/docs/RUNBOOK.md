...
**Node URL display:** Whenever a portal URL uses an inventory node FQDN under `cluster_domain`, the portal also shows a clickable equivalent in parentheses using the node's public IP when available, otherwise its private IP. Paths, schemes, and ports are preserved; Caddy lab hostnames remain unpaired because their DNS name already encodes or resolves to the intended edge address.

**Portal section ownership:** Quick-open cards provide navigation without repeating URL text. Network access shows only addressing/profile facts, the Caddy section shows only DNS/routing configuration, cluster URLs live with cluster details, and monitoring lists each service once with separate Caddy, public-IP, and private-IP columns. Credential tables contain credentials and notes only.
...
**Portal controls:** Successful HTTP Basic authentication to `/downloads/*` sets a portal marker so the header changes from **Login** to **Logout**. Logout clears that marker and invalidates the cached credentials on a best-effort basis; browsers can retain HTTP Basic credentials until the tab or browser is closed. The portal also provides quick-service filtering (`/` focuses it and `Esc` clears it), expand/collapse-all controls, a scroll-progress indicator, and fixed jump-to-top/jump-to-bottom buttons.
