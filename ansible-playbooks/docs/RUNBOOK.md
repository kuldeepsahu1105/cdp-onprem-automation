...
- **Play 25 does not require `:7183`:** `cm_verify_wait_scm_https_port` defaults to **false**; optional HTTPS port wait runs only after **`set_cm_api_url.yml`** when TLS transport is selected (`cm_protocol: https` / `cm_api_https_active`). Before Auto-TLS, bind-address discovery tries **HTTP :7180 first** (`cm_api_probe_https_first: false`); HTTPS :7183 is not probed or waited on when HTTP already answers.
