...
On **ipaserver**, confirm **`/etc/httpd/conf.d/zz-ipa-caddy-proxy.conf`** limits SUBSTITUTE to **`/ipa/ui`** and **`/ipa/modern-ui`** only (no broad **`<Location /ipa>`**); then **`apachectl configtest && systemctl reload httpd`**. Re-run Jenkins **PORTAL** to apply the Ansible template, or re-run **`16_setup_identity_client.yml`** — playbook **16** builds portal facts on **localhost**, applies **`configure_ipa_httpd_behind_caddy.yml`** on **`[ipaserver]`** (checksum compare, **`systemctl reload httpd`** after the template), probes **`/ipa/json`** on ipaserver, then enrolls workers — then **`16_setup_identity_client.yml --limit <host>`** if a single node was skipped.
