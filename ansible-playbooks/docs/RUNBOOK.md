...
Playbook **`12_setup_freeipa_server.yml`** (via **`11_identity_setup`**) skips **`ipa-server-install`** only when **`/etc/ipa/default.conf`** exists, **`ipactl status` rc = 0**, and output does **not** contain **IPA is not configured**. Otherwise it always runs install (preflight + **`ipa-server-install`**). Set **`ipa_server_install_force: true`** to force install even when healthy (rare). Repeated **partial installs** or manual cleanup can leave **broken state** (`ipactl` rc=4, leftover **`/var/lib/ipa`** / **`/etc/dirsrv`**, stale **`default.conf`**); fix the host manually or use **`12b_ipa_deep_recovery.yml`** before re-running identity.
