...
| **IDENTITY** | Phase 2: `11_identity_setup.yml` → **`12_setup_freeipa_server.yml`** (ipaserver) + **`16_setup_identity_client.yml`** (`all:!ipaserver` clients); ends with **`35_refresh`** when portal refresh is enabled | **`12`:** `/etc/krb5.conf.d/ansible-ccache-note.conf` + enctypes snippet; **`ipactl restart`** when enctypes snippet changes. **`16`:** same ccache drop-in on **ipaserver** and **`identity_client_hosts`** before enroll; **`sync_ipa_httpd_caddy_proxy_state_on_ipaserver.yml`** (cleanup or opt-in apply + **`/ipa/json`** preflight) | **`16`:** **SSSD restart** on a client when the ccache drop-in changes and **SSSD** is already active (enrolled hosts). Client enroll / kinit unchanged. Portal index refresh only — not a substitute for **PORTAL** Caddy compose |
