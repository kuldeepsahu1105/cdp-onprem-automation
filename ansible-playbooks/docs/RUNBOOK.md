...
**Caddy FreeIPA vhost:** When `[ipaserver]` is present, `http://ipa.<ops-ip-dashed>.<base>:81/` **`redir / /ipa/modern-ui/ permanent`** (Labs legacy-only setups use `/ipa/ui`) and reverse-proxies **HTTP** to `<ipaserver-fqdn>` with **`header_up Host`** and **path-matched `header_up Referer`** (`https://<ipaserver-fqdn>{request-path}` — required for legacy **`/ipa/js`**, **`/ipa/css`**, etc., not only **`/ipa/ui`**). Caddy **`handle_response`** rewrites upstream **`Location`** headers from the ipaserver FQDN back to the **`ipa.*`** vhost so browsers do not need **`/etc/hosts`**. **`/ipa/json`** (and **`/ipa/xml*`**, session paths) are handled first with fixed Referer **`/ipa/ui`** and **`header_up Accept-Encoding identity`** (no gzip stack on JSON-RPC). Tier A checks accept **301/308** on `/` and **200/301** on both UI paths; on `ipaserver`, Ansible verifies `/ipa/modern-ui/` and `/ipa/ui` with matching Referer headers. **Smoke (ops):** `curl -sS -o /dev/null -w '%{http_code}' -H "Host: ipa.<slug>.<base>" http://127.0.0.1:81/ipa/ui` and the same Host for **`/ipa/js/util.js`** — compare with direct `curl` to ipaserver loopback using **`Host: <ipaserver-fqdn>`** and Referer **`https://<ipaserver-fqdn>/ipa/js/util.js`**. **Default:** no ipaserver httpd edits — Caddy alone fronts the UI. Optional **`deployment_portal_ipa_httpd_proxy_enabled: true`** applies Apache **`mod_substitute`** on **`/ipa/ui`** and **`/ipa/modern-ui`** only (not **`/ipa/json`**) for embedded FQDN link rewrites.
