...
**CM remote parcel repositories:** Playbooks **25**, **31**, and **33** call `configure_cm_parcel_repo_api.yml`, which sets `REMOTE_PARCEL_REPO_URLS` to **pinned** CDH/ECS/CDV/CFM parcel and CSD archive directory URLs (`cdh_version`, `ecs_pvc_ds_version`, `cdv_version`, `cfm_version`). Default **`cm_parcel_repo_include_latest: false`** — use pinned version directories only. Never use `…/{latest}/parcels/` (invalid URI and not a real archive path). Optional float uses literal `…/latest/parcels/` via **`cm_parcel_repo_latest_segment: latest`**. CSD entries include **CDV** (`redhat8/yum` + `parcels/`) and **CFM2** (`redhat9/yum/tars/parcel` + `parcels/`). Scrub also drops legacy wizard URLs, `.jar` paths, merged garbage URLs (`7180/cmf`, double `http://`), and any URL containing `{` or `}`. Intel MKL (`…/mkl/latest`) remains a literal path segment, not `{latest}`. `cm_parcel_install_csd_repo_urls: false` prevents CM from re-adding CSD-derived spark/cdh6 URLs on scm-server restart.
...
If CM shows **no CDV/CFM remote repos** after a run, check: (1) `cm_parcel_repo_include_cdv_parcel` / `cm_parcel_repo_include_cfm_parcel` are true and `cm_repo_source: public`; (2) task **Parcel repo — default empty CSD archive directory URL list** ran because `cm_parcel_repo_include_csd_archive_dirs: false` (pinned CDV/CFM URLs from `cdv_parcel_*` / `cfm_parcel_*` are still merged when those include toggles are on); (3) `cm_parcel_csd_respect_service_toggles: true` with `base_cluster_install_services.dataviz` / `nifi` / `nifi_registry` false yields an empty auto-built list from `scm_csd_parcel_repo_urls.j2` — leave `cm_parcel_csd_respect_service_toggles` at default **false** or enable those service keys; (4) override with explicit `cm_remote_parcel_csd_repo_urls` in `group_vars/all.yml` or `-e`. Jenkins `ANSIBLE_GROUP_VARS_YAML` can set `cdv_version`, `cfm_version`, and `base_cluster_install_services`; parcel-repo toggles are in `group_vars/all.yml` today (not in `jenkins/ansible-group-vars-allowed-keys.yaml`).
...
**Caddy FreeIPA vhost:** When `[ipaserver]` is present, `http://ipa.<ops-ip-dashed>.<base>:81/` **`redir / /ipa/modern-ui/ permanent`** (Labs legacy-only setups use `/ipa/ui`) and reverse-proxies **HTTP** to `<ipaserver-fqdn>` with **`header_up Host`**, **path-matched `header_up Referer`** on UI assets (`https://<ipaserver-fqdn>{request-path}`), and **`header_up Referer` regexp rewrite** on **`/ipa/session/*`** (browser sends `http://ipa.<slug>:81/...`; upstream requires **`https://<ipaserver-fqdn>/...`**). Caddy **`handle_response`** rewrites upstream **`Location`** and **`Set-Cookie`** domains from the ipaserver FQDN to the **`ipa.*`** vhost (without this, browsers drop session cookies and **`POST /ipa/session/json`** returns **401**). **`/ipa/json`** and **`/ipa/xml*`** use Referer **`https://<ipaserver-fqdn>/ipa`** and **`Accept-Encoding identity`** (no gzip stack); **`/ipa/session/*`** is separate from JSON-RPC. **`GET /ipa/session/login_kerberos` → 401** is expected when **`ipa_httpd_disable_browser_krb_negotiate: true`** — use password login. Tier A checks accept **301/308** on `/` and **200/301** on both UI paths. **Smoke (ops):** `curl -sS -o /dev/null -w '%{http_code}' -H "Host: ipa.<slug>.<base>" http://127.0.0.1:81/ipa/ui` and **`/ipa/js/util.js`**. **Default:** no ipaserver httpd edits — Caddy alone fronts the UI. Optional **`deployment_portal_ipa_httpd_proxy_enabled: true`** adds Apache **`mod_substitute`** on UI paths only (not **`/ipa/json`**).
...
**Where to open the UI:** **`http://ipa.<ops-ip-dashed>.<base>:81/`** (Caddy on ops — redirects to **`/ipa/modern-ui/`**) or **`https://ipaserver.<domain>/ipa/modern-ui/`** (direct; resolve via lab DNS or **`/etc/hosts`**). Set **`ipa_httpd_disable_browser_krb_negotiate: false`** only if you need browser Kerberos SSO on the **ipaserver** FQDN (stock **`ipa.conf`** GSSAPI).
