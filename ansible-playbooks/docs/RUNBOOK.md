...
|---|---|---|---|
| **PORTAL** | `10_setup_deployment_portal.yml` (+ optional `35_refresh` at end of later phases) | Re-renders Caddy, portal index, pgAdmin compose; **does not** install **`zz-ipa-caddy-proxy.conf`** unless **`deployment_portal_ipa_httpd_proxy_enabled: true`** | **Caddy** (and pgAdmin/monitoring containers) **force-recreate** when rendered templates change; **httpd reload on ipaserver** only when the opt-in HTTPD proxy flag is **true** and snippets/rewrite changed |
| **IDENTITY** | Phase 2: `11_identity_setup.yml` → **`12_setup_freeipa_server.yml`** (ipaserver) + **`16_setup_identity_client.yml`** (`all:!ipaserver` clients); ends with **`35_refresh`** when portal refresh is enabled | **`12`:** `/etc/krb5.conf.d/ansible-ccache-note.conf` + enctypes snippet; **`ipactl restart`** when enctypes snippet changes. **`16`:** same ccache drop-in on **ipaserver** and **`identity_client_hosts`** before enroll; **no** ipaserver httpd/Caddy proxy by default | **`16`:** **SSSD restart** on a client when the ccache drop-in changes and **SSSD** is already active (enrolled hosts). Client enroll / kinit unchanged. Portal index refresh only — not a substitute for **PORTAL** Caddy compose |

**Operator takeaway:** Re-run **IDENTITY** to push fleet **`ansible-ccache-note.conf`**, refresh FreeIPA server krb5 snippets (with **`ipactl restart`** when enctypes change), and re-enroll or verify clients. Re-run **PORTAL** for Caddy/portal stack updates and, only if explicitly enabled, ipaserver **`zz-ipa-caddy-proxy.conf`** + **httpd reload**.
