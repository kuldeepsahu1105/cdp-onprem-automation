...
**Operator access hub:** After `10_setup_deployment_portal.yml` or `35_refresh_deployment_portal.yml`, the index includes an **Operator access** panel with CM, database, IPA/AD, enabled CDP services (Ranger/Knox/Hue), ECS, and optional Jenkins URLs — values come from Ansible vars at render time (not committed secrets). Optional SSH downloads: set `deployment_portal_expose_ssh_keys: true` (or `auto` with `deployment_portal_basic_auth_enabled: true`). When the controller has both Terraform `sshkey.pem` and a distinct Auto-TLS key (for example `~/.ssh/id_rsa`), the portal exposes two labeled links; a single key exports once. Protect `/downloads/*` with `deployment_portal_basic_auth_enabled: true` when exporting keys on untrusted networks. Re-run portal refresh after password changes in group_vars or Jenkins `ANSIBLE_GROUP_VARS_YAML`.
...
| 3 | **`stop` from Jenkins:** set **`EC2_STARTSTOP_CONFIRM=true`**. Manual stop on ipaserver: `/root/<prefix>_cldr_ec2_strt_stp.sh stop <environment> <group> …` prompts **`yes`** (`<prefix>` = sanitized **`deployment_name_prefix`** / Jenkins **`ENVIRONMENT`**; override **`ec2_startstop_script_name_prefix`**; full path **`ec2_startstop_script_path`**). |
| 4 | Example on ipaserver: `/root/development_cldr_ec2_strt_stp.sh describe development pvcbase_worker pvcecs_worker` (basename from **`ec2_startstop_script_basename`** in `group_vars/all.yml`) |
...
| **FreeIPA partial install** (`ipa-client-install` / decompress errors; log shows `Using existing certificate '/etc/ipa/ca.crt'` without a healthy server) | On **ipaserver** as root: `ipactl status` and `test -f /etc/ipa/default.conf`. If `default.conf` is missing but `/etc/ipa/ca.crt` or `/var/lib/ipa` exists, **do not** re-run `ipa-server-install` until cleanup completes. Run `ipa-server-install --uninstall --unattended` (or interactive `--uninstall`). Then `ansible-playbook -i inventory.ini 12_setup_freeipa_server.yml --limit ipaserver`. Playbook **12** fails fast on partial debris instead of layering a new install. |
| FreeIPA configured but stopped (`default.conf` present, `ipactl` not RUNNING) | Playbook **12** skips `ipa-server-install` and runs `ensure_ipa_kdc_services.yml` (`ipactl start`). Manual: `ipactl start` on ipaserver. |
