...
- **Play 25 does not require `:7183`:** `cm_verify_wait_scm_https_port` defaults to **false**; optional HTTPS port wait runs only when TLS is already expected (`cm_api_use_https` or Auto-TLS already in CM config).
