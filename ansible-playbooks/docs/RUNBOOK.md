...
- **CM Postgres `psql` host (`postgres_ensure_cm_db_psql_host`):** plays **24** / **29** delegate database checks to `postgres_inventory_host`. Leave unset for auto (`127.0.0.1` on the DB node when `ansible_control_reachability` resolves to public; `postgres_host_fqdn` when in-VPC). `-e postgres_ensure_cm_db_psql_host=<private-ip>` only helps when you intentionally match CM JDBC from an in-VPC controller — external laptops cannot reach VPC-private IPs; use auto or `-e postgres_ensure_cm_db_psql_host=127.0.0.1`.
...
4. Re-run: `cd ansible-playbooks && ansible-playbook -i inventory.ini 16_setup_identity_client.yml --limit pvcecs-worker4` (set **`ANSIBLE_PRIVATE_KEY`** or key in playbook dir). After a gzip fix, you do **not** need to uninstall on workers that already have **`/etc/ipa/default.conf`** (they skip install).
