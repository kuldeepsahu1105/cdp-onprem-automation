...
| Issue | Action |
|---|---|
| **RHEL package install fails after DNS update** (`Could not resolve host: rhui.<region>.aws.ce.redhat.com`) | The client resolver points to FreeIPA first, but IPA forwarding to the AWS VPC resolver is not healthy. Current automation installs identity-client packages before DNS cutover, reconciles `ipa dnsconfig-mod --forwarder=<vpc-dns> --forward-policy=first`, and waits for the regional RHUI name to resolve before enrollment. Re-run the full **IDENTITY** stage so playbook **12** repairs the IPA forwarder before **13/16**. On ipaserver verify `ipa dnsconfig-show --all --raw` and `dig +short @127.0.0.1 rhui.<region>.aws.ce.redhat.com A`. |
