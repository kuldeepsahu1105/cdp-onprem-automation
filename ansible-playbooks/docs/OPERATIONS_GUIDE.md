...
**cAdvisor (container-level metrics):** Already part of the monitoring Docker Compose stack (`monitoring_docker-compose.yml.j2`, service `cadvisor`, image `monitoring_cadvisor_image`) — no separate enable flag; it runs whenever `monitoring_stack_enabled` (or an already-deployed stack) is true. Mounts `/`, `/var/run`, `/sys`, and `/var/lib/docker` read-only from the ops host to report per-container CPU/memory/network/disk-IO for every container on that host (Caddy, pgAdmin, Prometheus, Grafana, Alertmanager, blackbox_exporter, and cAdvisor itself), scraped by Prometheus as the `cadvisor` job (`container_*` metrics, e.g. `container_last_seen`, `container_cpu_usage_seconds_total`). Exposed on host port **`monitoring_cadvisor_host_port`** (default **8089**) and via Caddy `/cadvisor/` (path route) or the `cadvisor.<ip>.<base>` lab vhost when `caddy_vhost_enabled` is true.
...
|-----------------|--------------------------------------|---------------------------------------------|---------------------------------------|----------------------------------------|
| **ipaserver** (ops / portal host) | Yes (systemd on host) | Yes | Yes — **only** containers on this host (portal stack + monitoring stack) | Probes run **from** blackbox on ops; targets include portal Caddy `:81`, pgAdmin/Grafana/Prometheus containers, IPA/CM/ECS URLs |
| **cldr-mngr** | Yes | Yes | No — CM is RPM/Java, not Docker on this node | CM UI `:7180`, CM API `:7183/api/version`, PostgreSQL `:5432` (TCP) |
| **base-masters** / **base-workers** | Yes | Yes | No — CDP roles are packages, not ops Docker | Knox gateway **TCP :8443** when `base_cluster_install_services.knox` is true (master private IP) |
| **ecs-masters** / **ecs-workers** | Yes | Yes | No — Kubernetes/containerd on ECS is **not** scraped by ops cAdvisor | ECS console HTTPS when ECS is in inventory (`https://console.<ecs_app_domain>`) |

cAdvisor is **not** deployed as a daemon on every cluster node by design: it only inspects the Docker daemon on the **ops host** where the portal/monitoring Compose stack runs. **“All nodes”** for CPU/memory/disk/network means **`node_exporter` + `process_exporter`** on every group in `monitoring_node_exporter_host_groups` / `monitoring_process_exporter_host_groups` (defaults: all six groups above). Container-level dashboards for the ops stack use cAdvisor; cluster workload health uses host/process series plus blackbox probe success (`probe_success`).
