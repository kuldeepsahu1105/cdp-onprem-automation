...
|--------|----------------|-----|
| All `node_exporter` targets DOWN; no `process_exporter` job | Prometheus synced before **`enroll_monitoring_exporters.yml`** (common when play **10** ran with `-e monitoring_stack_enabled=false` while monitoring routes were already active), or stale `prometheus.yml` | Re-run **`enroll_monitoring_exporters.yml`** via play **32**/**35**/**10** (enrollment gate: `monitoring_stack_enabled` **or** `deployment_portal_monitoring_routes_enabled`). Order: facts → enroll → sync. Confirm `/opt/cldr-monitoring/prometheus.yml` contains `job_name: process_exporter`. |
...
1. **Ops host (portal / monitoring Compose)** — service `cadvisor` in `monitoring_docker-compose.yml.j2` (`monitoring_cadvisor_image`, container `cldr-mon-cadvisor`). Runs whenever `monitoring_stack_enabled` (or an already-deployed stack) is true. Mounts `/`, `/var/run`, `/sys`, and `/var/lib/docker` read-only from the ops host (Caddy, pgAdmin, Prometheus, Grafana, Alertmanager, blackbox_exporter, etc.). The pinned cAdvisor release supports Docker's containerd-backed `overlayfs` image store; older `v0.49.1` emitted `failed to identify the read-write layer ID` and exposed only root-cgroup metrics on that configuration. Prometheus job **`cadvisor`** scrapes `cldr-mon-cadvisor:8080` on the Docker network (`cadvisor_scope: ops`). UI on host port **`monitoring_cadvisor_host_port`** (default **8089**) and via Caddy `/cadvisor/` or the `cadvisor.<ip>.<base>` lab vhost when `caddy_vhost_enabled` is true.

2. **ECS container hosts** — playbook **`enroll_monitoring_exporters.yml`** imports `common_tasks/install_cadvisor_host.yml`, which runs the same image as a **standalone Docker container** (`monitoring_cadvisor_host_container`, default `cldr-host-cadvisor`) on every host in **`monitoring_cadvisor_host_groups`** (default: `ecs-masters` and `ecs-workers`). Host publish port **`monitoring_cadvisor_cluster_port`** (default **19180** → container **8080**) avoids common CDP/YARN ports such as **8088**. Extra read-only mounts for `/var/lib/containerd` and `/dev/disk`, privileged mode, and `/dev/kmsg` provide Docker/containerd visibility. Hosts without a working Docker daemon are skipped and omitted from Prometheus targets. Prometheus job **`cadvisor_host`** adds one static target per successfully enrolled host at `<private_ip>:19180` with `instance` and `inventory_group` labels (`cadvisor_scope: host`). Verify: `docker ps --filter name=cldr-host-cadvisor` and `curl -s http://127.0.0.1:19180/metrics | grep 'container_last_seen.*name='` on an ECS host; check Prometheus **Status → Targets** for `job="cadvisor_host"` and the Grafana **cAdvisor — Cluster Containers** dashboard.
...
|-----------------|--------------------------------------|---------------------------------------------|---------------------------------------|----------------------------------------|
| **ipaserver** (ops / portal host) | Yes (systemd on host) | Yes | Yes — monitoring Compose **`cadvisor`** job on ops (`:8089`) | Probes run **from** blackbox on ops; targets include portal Caddy `:81`, pgAdmin/Grafana/Prometheus containers, IPA/CM/ECS URLs |
| **cldr-mngr** | Yes | Yes | No — no workload container runtime by default | CM UI `:7180`, CM API `:7183/api/version`, PostgreSQL `:5432` (TCP) |
| **base-masters** / **base-workers** | Yes | Yes | No — no workload container runtime by default | Knox gateway **TCP :8443** when `base_cluster_install_services.knox` is true (master private IP) |
...
**“All nodes”** for CPU/memory/disk/network means **`node_exporter` + `process_exporter`** on every group in `monitoring_node_exporter_host_groups` / `monitoring_process_exporter_host_groups`. Container-level metrics apply only where containers run: Compose cAdvisor on the ops host and standalone host cAdvisor on ECS masters/workers.
...
**blackbox_exporter (HTTP/TCP probes):** Docker service `blackbox` on the ops host (`monitoring_blackbox_exporter_image`, container `cldr-mon-blackbox`, host publish **`monitoring_blackbox_exporter_host_port`** default **19115** → container **9115**). Config: `{{ monitoring_config_dir }}/blackbox.yml` (modules `http_2xx`, `http_2xx_ssl` with `insecure_skip_verify` for lab CM/ECS TLS, `tcp_connect`). Probe list is rendered in `monitoring_blackbox_probe_targets.j2` into `deployment_portal_blackbox_probe_targets` during `build_deployment_portal_facts.yml` **after** Caddy vhost URL facts exist (so lab vhost hostnames are included). Prometheus jobs `blackbox_<module>` use `/probe` with relabel to `cldr-mon-blackbox:9115`. Auto probes include: portal Caddy and path routes (`/grafana/`, `/prometheus/`, `/alertmanager/`, `/cadvisor/`); CM HTTPS API/UI and Postgres TCP on `cldr-mngr`; FreeIPA `/ipa/json`; pgAdmin/Grafana/Prometheus/**Alertmanager**/ops cAdvisor on the Docker network; Caddy vhost URLs when enabled; CDP Runtime base cluster endpoint URLs from `deployment_portal_base_cluster_portal_urls`; Knox TCP; ECS console/CDP API and optional ECS data-service URLs. Toggle auto groups with `monitoring_blackbox_probe_caddy_paths_enabled`, `monitoring_blackbox_probe_caddy_vhosts_enabled`, `monitoring_blackbox_probe_base_cluster_enabled`, and `monitoring_blackbox_probe_ecs_data_services_enabled`. Extend with `monitoring_blackbox_extra_probes` (list of `{module, target, labels}`) or disable entirely with `monitoring_blackbox_exporter_enabled: false`. Verify: Prometheus **Status → Targets** for `job=~"blackbox_.*"` and `probe_success==1`, or `curl -s "http://127.0.0.1:19115/probe?target=https://<cm-private-ip>:7183/api/version&module=http_2xx_ssl" | head` on the ops host.
