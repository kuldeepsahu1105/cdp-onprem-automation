...
**Auto-TLS reports `/etc/ipa/ca.crt` missing on cldr-mngr:** This is an incomplete FreeIPA client state, even if `/etc/ipa/default.conf` exists. Current automation runs `ipa-certupdate` and requires a non-empty CA bundle after every client enrollment and again on the CM host immediately before `generateCmca`. Update to current `main` and rerun **CM_TLS_KRB_LDAP**; if repair still fails, rerun **IDENTITY** and verify DNS, time synchronization, `ipa ping`, and `ipa-certupdate` directly on cldr-mngr.
