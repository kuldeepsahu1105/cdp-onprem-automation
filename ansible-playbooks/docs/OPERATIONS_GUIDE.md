...
CSD JARs for DataViz / NiFi / NiFi Registry are listed in `scm_csds_effective` during `24_start_cm.yml` (RHEL CM): built from `scm_csds_urls.j2` when `scm_csds` is empty, using `cdv_version`, `cfm_version`, and related vars. Play 24 `get_url`s each URL; DATAVIZ yum failures ignore errors and `common_tasks/download_cdv_dataviz_csd.yml` falls back to the matching `cdv/<version>/parcels/DATAVIZ-*-el9.parcel` and extracts the CSD jar into `/opt/cloudera/csd`. By default `csd_respect_service_toggles: true` downloads only CSDs for services enabled in `base_cluster_install_services`. Set e.g. `cdv_version: "8.1.5"` and update `cdv_dataviz_csd_jar` to match the archive jar name, or pass an explicit `scm_csds` list of full jar URLs (replacing the auto-built list).
...
**Auto-TLS reports `/etc/ipa/ca.crt` missing on cldr-mngr:** Auto-TLS needs the FreeIPA CA bundle when `use_freeipa_for_crt_mgmt=true`. Current automation copies the authoritative `/etc/ipa/ca.crt` from the inventory `ipaserver` host to cldr-mngr when missing, then verifies complete CM-host enrollment before `generateCmca`: remote FQDN, non-empty `/etc/ipa/default.conf` and CA bundle, `/etc/krb5.keytab`, the matching `host/<fqdn>@REALM` principal, host-keytab `kinit`, and authenticated `ipa ping`. The same enrollment gate runs after every FreeIPA client enrollment. Update to current `main` and rerun **CM_TLS_KRB_LDAP**; if validation fails, rerun **IDENTITY** and use the reported host/file/principal details to repair the specific client.
