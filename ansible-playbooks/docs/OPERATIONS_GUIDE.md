...
**Auto-TLS reports `/etc/ipa/ca.crt` missing on cldr-mngr:** Auto-TLS needs the FreeIPA CA bundle when `use_freeipa_for_crt_mgmt=true`, but copying that public CA does not require CM to be an enrolled IPA client. Current automation copies the authoritative `/etc/ipa/ca.crt` from the inventory `ipaserver` host to cldr-mngr when missing, then requires a non-empty regular file before `generateCmca`. The **IDENTITY** phase separately runs `ipa-certupdate` and validates complete FreeIPA client enrollment. Update to current `main` and rerun **CM_TLS_KRB_LDAP**; if repair still fails, verify `[ipaserver]`, the source CA file, and Ansible SSH access to both hosts.
