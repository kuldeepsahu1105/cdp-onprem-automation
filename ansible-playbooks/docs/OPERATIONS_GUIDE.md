...
|-----------------|--------------------------------------|---------------------------------------------|---------------------------------------|----------------------------------------|
| **ipaserver** (ops / portal host) | Yes (systemd on host) | Yes | Yes — Compose **`cadvisor`** job on ops (`:8089`); host enrollment is also attempted and skipped if Docker is unavailable | Probes run **from** blackbox on ops; targets include portal Caddy `:81`, pgAdmin/Grafana/Prometheus containers, IPA/CM/ECS URLs |
| **cldr-mngr** | Yes | Yes | Enrollment attempted; skipped when Docker is unavailable | CM UI `:7180`, CM API `:7183/api/version`, PostgreSQL `:5432` (TCP) |
| **base-masters** / **base-workers** | Yes | Yes | Enrollment attempted; skipped when Docker is unavailable | Knox gateway **TCP :8443** when `base_cluster_install_services.knox` is true (master private IP) |
