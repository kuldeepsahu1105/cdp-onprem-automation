...
| Group defaults | `group_vars/all.yml` | e.g. `deployment_portal_http_port`, `deployment_portal_ipa_httpd_proxy_enabled` (default **false** — Caddy-only IPA UI), `deployment_portal_url_verify_status_codes`, `caddy_vhost_enabled`, `deployment_portal_basic_auth_enabled` (HTTP auth on `/downloads/*`) |
