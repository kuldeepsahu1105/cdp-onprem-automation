...
| `postgres_db_host_psql_present` | `ensure_cm_postgres_databases.yml` | `true` when `/usr/pgsql-<ver>/bin/psql` (RHEL) or Debian versioned path exists, or `which psql` succeeds on `postgres_inventory_host` — skips the entire PGDG/client install block. |
| `postgres_db_host_el_major` | `ensure_cm_postgres_databases.yml` | RHEL major (`8` or `9`) from `hostvars[postgres_inventory_host].ansible_distribution_major_version` after delegated `setup` — never play-host/controller facts (AL2023 → `2023` breaks PGDG). |
| `postgres_db_host_architecture` | `ensure_cm_postgres_databases.yml` | DB host `ansible_architecture` mapped to PGDG reporpms segment (`x86_64`, `aarch64`). |
