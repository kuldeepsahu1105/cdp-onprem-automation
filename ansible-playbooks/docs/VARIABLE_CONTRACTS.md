...
| `krb5_cm_managed_krb5_conf` | `group_vars/all.yml` (default **`true`**) | CM **`KRB_MANAGE_KRB5_CONF`** — CM regenerates **`/etc/krb5.conf`** on cluster hosts; set **`false`** to keep host/IPA-managed krb5 |
| `krb5_cm_libdefaults_safety_valve` | `group_vars/all.yml` | CM **`KRB_LIBDEFAULTS_SAFETY_VALVE`** when managed krb5 is on; defaults to **`krb5_libdefaults_default_ccache_commented_line`** (play **30**) |
...
| `ipa_client_has_default_conf`, `ipa_client_has_partial_state`, `ipa_client_recovery_requires_uninstall` | `detect_ipa_client_install_state.yml` | Gates install and preflight; uninstall requires concrete client sysrestore markers |
