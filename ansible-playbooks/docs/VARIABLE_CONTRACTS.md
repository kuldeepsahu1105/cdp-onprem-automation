...
| `postgres_db_host_el_major` | `ensure_cm_postgres_databases.yml` | RHEL major from delegated DB host facts — PGDG `get_url` on `postgres_inventory_host` (not controller paths). |
| `postgres_db_host_architecture` | `ensure_cm_postgres_databases.yml` | DB host `ansible_architecture` for PGDG RPM URL (e.g. `x86_64`). |
