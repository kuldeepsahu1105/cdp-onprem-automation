...
| `_postgres_cm_db_tcp_probe_hosts` | `ensure_postgresql_running.yml` (localhost) | Ordered TCP probe list: `postgres_ensure_cm_db_psql_host_effective` plus `postgres_host_fqdn` / DB host `private_ip` / `ansible_host` when auto host is `127.0.0.1` (PG may not listen on loopback). |
