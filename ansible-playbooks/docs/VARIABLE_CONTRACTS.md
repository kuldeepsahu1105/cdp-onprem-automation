...
**`27_setup_cm_autotls.yml` generateCmca gate:** captures **`cm_autotls_setup_intent`** from inventory **`autotls_enabled`** before **`set_cm_api_url.yml`** (HTTPS probes may also set runtime `autotls_enabled`). Skips **`generateCmca`** when **`cm_autotls_api_enabled`** and agent truststore exist, or when manual HTTPS on **`cm_https_port`** with **`autotls_enabled: false`** (post-#169 HTTPS-only skip). Sets **`cm_autotls_generate_cmca_skip`** / **`cm_api_https_on_cm_port`** helpers.
...
| `postgres_db_host_psql_present` | `ensure_cm_postgres_databases.yml` | `true` when delegated `stat` finds `/usr/pgsql-<ver>/bin/psql` or Debian versioned path on `postgres_inventory_host`, or `which psql` succeeds — **before** OS-family / PGDG logic; skips the entire PGDG/client install block. |
| `postgres_db_host_el_major` | `ensure_cm_postgres_databases.yml` | RHEL major (`8` or `9`) from delegated `rpm -E '%{rhel}'` on the DB host (fallback: `ansible_distribution_major_version` only when `ansible_distribution` is `RedHat`, then `/etc/redhat-release`) — never polluted AL2023 `2023` in `hostvars`. |
