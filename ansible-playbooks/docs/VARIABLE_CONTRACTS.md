...
**`27_setup_cm_autotls.yml` generateCmca gate:** captures **`cm_autotls_setup_intent`** from inventory **`autotls_enabled`** (default **`true`** in `group_vars/all.yml`) before **`set_cm_api_url.yml`** (HTTPS probes may also set runtime `autotls_enabled`). Skips **`generateCmca`** when **`cm_autotls_api_enabled`** and agent truststore exist, or when manual HTTPS on **`cm_https_port`** with **`autotls_enabled` explicitly `false`** (post-#169 HTTPS-only skip). Sets **`cm_autotls_generate_cmca_skip`** / **`cm_api_https_on_cm_port`** helpers. On successful **`generateCmca`** (or **`cm_autotls_force_run`** with a successful POST), sets **`autotls_applied_this_run`** and **`cm_restart_after_autotls_this_run`** → **`apply_cm_restart_after_config.yml`** (`restart_cm_services.yml` systemctl) and cluster-wide agent restart via **`reconcile_cm_agents.yml`** / **`restart_cm_agent_if_needed.yml`**. Pure skip (truststore present, no POST; manual TLS with **`autotls_enabled: false`**) does not restart CM server or agents.
...
| `postgres_db_host_os_family` | `ensure_cm_postgres_databases.yml` | `RedHat` or `Debian` on the **play host** — delegated `rpm -E '%{rhel}'` (`8`/`9` → RedHat), trustworthy `hostvars[postgres_inventory_host].ansible_distribution`, then play-host facts when the play runs on the DB host. Used in `when:` for client packages; do not reference `hostvars[postgres_inventory_host].ansible_os_family` directly (HostVarsVars attribute errors on localhost plays). |
| `postgres_db_host_psql_present` | `ensure_cm_postgres_databases.yml` | `true` when delegated `stat` finds `/usr/pgsql-<ver>/bin/psql` or Debian versioned path on `postgres_inventory_host`, or `which psql` succeeds — **before** OS-family / PGDG logic; skips the entire block (OS-family detection, assert, PGDG repo, and client install). |
| `postgres_db_host_el_major` | `ensure_cm_postgres_databases.yml` | RHEL major (`8` or `9`) from delegated `command` `argv: [rpm, -E, '%{rhel}']` on the DB host (`rhel_major_cmd.stdout`; fallback: `ansible_distribution_major_version` only when `ansible_distribution` is `RedHat`, then `/etc/redhat-release`) — never polluted AL2023 `2023` or literal `%{rhel}` in `hostvars`. Do not use `cmd:` with a YAML list (`cmd` is a string-only parameter). |
| `postgres_db_host_architecture` | `ensure_cm_postgres_databases.yml` | DB host `uname -m` mapped to PGDG reporpms segment (`x86_64`, `aarch64`); fallback to `ansible_architecture` when distribution is `RedHat`. |
