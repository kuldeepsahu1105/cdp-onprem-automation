...
# Full SG name: ${ENVIRONMENT}-${SG_NAME_SUFFIX}
SG_NAME_SUFFIX="${SG_NAME_SUFFIX:-pvc_cluster_sg}"
SG_NAME="${SG_NAME:-${ENVIRONMENT}-${SG_NAME_SUFFIX}}"
ALLOW_ALL="${ALLOW_ALL:-false}"
ALLOWED_CIDRS="${ALLOWED_CIDRS:-[\"137.83.231.109/32\", \"137.83.231.11/32\", \"208.127.31.110/32\", \"208.127.31.11/32\", \"139.180.248.227/32\", \"54.254.32.236/32\"]}"
ALLOWED_PORTS="${ALLOWED_PORTS:-[22,443,80,7180,7183,7182]}"

# --- New keypair (only when CREATE_KEYPAIR=true) ---
# Full AWS key name: ${ENVIRONMENT}-${KEYPAIR_NAME_SUFFIX}
KEYPAIR_NAME_SUFFIX="${KEYPAIR_NAME_SUFFIX:-pvc-new-keypair}"
KEYPAIR_NAME="${KEYPAIR_NAME:-${ENVIRONMENT}-${KEYPAIR_NAME_SUFFIX}}"
...
# --- Terraform state (local; Jenkins persists under holautosa HOL_AUTO_EXEC_DIR) ---
HOL_AUTO_EXEC_DIR="${HOL_AUTO_EXEC_DIR:-/home/${CREDENTIALS_USER:-holautosa}/HOL_AUTO_EXEC_DIR}"
TF_STATE_BACKEND="${TF_STATE_BACKEND:-local}"
