...
- **pgAdmin:** Caddy upstream must use compose service name **`pgadmin:80`** (not `container_name` — wrong DNS → **502**). Publish **`0.0.0.0:5050:80`** when SG allows direct UI. After upgrading Caddyfile/compose, re-run **PORTAL** or `docker compose up -d --force-recreate pgadmin caddy` in `deployment_portal_config_dir`. `verify_deployment_portal_pgadmin.yml` fails PORTAL with `docker logs` if the container is down. Optional `deployment_portal_pgadmin_debug_logs: true` dumps logs on success. Tier B from Jenkins: GET `http://<ops-public-ip>:8088/` with **`Host: pgadmin.<slug>.<base>`** — not only `:5050`.
