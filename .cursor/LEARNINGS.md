...
- **CM UI hostname `cm.<dashed-ops-ip>.pvc.cloudera-labs.com` is Caddy-only:** use **`http://…:8088`** (portal listener). **`https://cm.<slug>.<base>:7183` will not work** — nothing listens on 7183 on that DNS name. Direct CM HTTPS is **`https://<cldr-mngr-fqdn-or-public-ip>:7183`** (VPC/SG), not the Caddy vhost hostname on CM ports.
...
- Smoke: `http://<ops-public-ip>:8088/`. Vhost FQDN: `portal.<dashed-public-ip>.pvc.cloudera-labs.com` (dashes, not dots in IP segment).
