...
| FreeIPA Caddy **`freeipa.<ip>...`**, `redir / /ipa/ui`, HTTP upstream + **Referer** | Vhost key **`ipa`**; **`redir / /ipa/modern-ui/`**; **`@ipa_legacy`** + **`@ipa_session`** (Referer host rewrite); **`handle_response`** **`Set-Cookie`** + **`Location`** → **`ipa.*`** | Session/login **401** if cookies stay on ipaserver FQDN; **`login_kerberos` 401** OK when browser SPNEGO disabled. |
...
| ECS **`*.apps.ecs.<ip>...`** wildcard → ECS master **:80** | **No ECS Caddy vhost** — `https://console-cdp.<ecs_app_domain>` in portal/index (`ecs_console_host_prefix`) | Wildcard app ingress not proxied on ops Caddy; `apps_wildcard` hint in portal context only. |
| Knox **`knox.<ip>...`** Caddy vhost | Not implemented | Future: add `knox` to `caddy_vhost_service_names` + vhost block when Knox URL known. |

## Portal / Caddy

- **Caddy edge port** `deployment_portal_http_port` (default **81**) on ops — hostnames `portal.*`, `pgadmin.*`, `grafana.*`, `ipa.*`, etc. **CM and ECS are not on Caddy** — use `https://cldr-mngr.<domain>:7183` and `https://console-cdp.<ecs_app_domain>` (or `ecs_console_host_prefix`).
...
- **IPA dual UI:** Caddy vhost `ipa.<dashed-ops-ip>.pvc.cloudera-labs.com:81` **`redir / /ipa/modern-ui/`**; **`reverse_proxy` HTTP** to `<ipaserver-fqdn>:80` with `header_up Host`, path-matched **Referer** on UI paths, **Referer regexp** on **`/ipa/session/*`**, and **`handle_response`** **Location** + **Set-Cookie** rewrite to **`ipa.*`** (legacy **`@ipa_legacy`** includes **`/ipa/js`** et al.). **Default:** `deployment_portal_ipa_httpd_proxy_enabled: false` — PORTAL/**16** run **`remove_ipa_httpd_caddy_proxy_on_ipaserver.yml`**. Opt-in **`configure_ipa_httpd_behind_caddy.yml`** (never **`mod_substitute`** on `/ipa/json`). **`Accept-Encoding: identity`** on **`/ipa/json`** / **`/ipa/xml*`** only. **IPA UI:** **`ipa_httpd_disable_browser_krb_negotiate: true`** → **`zz-ipa-disable-browser-krb.conf`**; **`login_kerberos` 401** is normal — password login via **`/ipa/session/json`**. **`ipa`/`ipa-client-install`** use Ansible **`KRB5CCNAME=FILE:…`**. CM Tier A uses **`probe_cm_manager_ui_http.yml`** on cldr-mngr, not Caddy.
