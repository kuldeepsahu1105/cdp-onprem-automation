...
- Caddy/portal stack runs only when **`monitoring_stack_enabled`** (and `deployment_portal_enabled`) — `deployment_portal_caddy_effective` in `resolve_deployment_portal_caddy_enabled.yml`.
