...
- **IPA dual UI:** Caddy vhost `ipa.<dashed-ops-ip>.pvc.cloudera-labs.com:81` **`redir / /ipa/modern-ui/`**; **`reverse_proxy` HTTP** to `<ipaserver-fqdn>:80` with `header_up Host` + **Referer** (cloudera-labs/openshift). **Default:** `deployment_portal_ipa_httpd_proxy_enabled: false` — PORTAL/**16** run **`remove_ipa_httpd_caddy_proxy_on_ipaserver.yml`** (absent **`zz-ipa-caddy-proxy.conf`**, revert **`ipa-rewrite`** markers, **httpd reload**). Opt-in flag **true** applies snippets via **`configure_ipa_httpd_behind_caddy.yml`** (never **`mod_substitute`** on `/ipa/json`). Caddy sends **`Accept-Encoding: identity`** on `/ipa/json`. FILE ccache documented in **`/etc/krb5.conf.d/ansible-ccache-note.conf`** (`KRB5CCNAME`, not active `default_ccache_name`). Fleet **`ensure_krb5_default_ccache_commented.yml`** keeps **`#default_ccache_name = KEYRING:persistent:%{uid}`** in **`/etc/krb5.conf`** (after **`ipa-client-install`** on clients). CM Tier A uses **`probe_cm_manager_ui_http.yml`** on cldr-mngr, not Caddy.
