...
- **Jenkins outside VPC:** `detect_ansible_control_reachability` → `ansible_control_reachability: public` (`ANSIBLE_CONTROL_VIA_JENKINS=1`, `jenkins_override.yml`). Never `uri`/CM/portal checks via `172.31.x` from the controller. **CM API probes run on the controller** against `ansible_host` (HTTPS `:7183` first), not delegated `127.0.0.1` on cldr-mngr (EC2 hairpin). Optional Caddy: `http://cm.<dashed-ip>.pvc.cloudera-labs.com:81/api/version` when `cm_config_api_via_caddy_proxy`. In-VPC localhost plays still delegate to **cldr-mngr** (`select_cm_api_probe_host`: `127.0.0.1`, then ansible_host/FQDN). VPN/bare-metal controller → `private`.
...
|--------------|-------------------|--------------|
| `proxy_host` = `cm.<reverse_proxy_public_ip_dashed>.pvc.cloudera-labs.com`; Caddy on reverse proxy **:80** | `cm.<dashed-ip>.pvc.cloudera-labs.com` on ops Docker Caddy **:81** (`caddy_vhost_urls.j2`) | Port **81** (`deployment_portal_http_port`) is intentional (SG/docs); hostname shape matches. |
| **`reverse_proxy` play:** template `cloudera_manager.caddyfile.j2` → CM **:7180**, reload Caddy; after Auto-TLS → **:7183** HTTPS upstream | **`36_provision_cm_caddy_reverse_proxy.yml`** after `24_start_cm` (sync Caddy CM block + reload); **`27` + `35_refresh`** flips upstream when `autotls_enabled` | PORTAL `10` bootstraps stack; **36** updates CM proxy once scm-server listens (labs timing). |
| `frontend_url` play after proxy + license (via `proxy_host` API) | **`26_setup_cm_license.yml`** → `apply_cm_caddy_load_balancer.yml` after license (API via Caddy when **36** ran) | Not deferred to CM_TLS-only refresh. |
...
| `frontend_url` via `cm_config` to **`https://proxy_host`** (play after proxy) | `http://cm.<slug>.<base>:81` via `apply_cm_caddy_load_balancer.yml` after `build_deployment_portal_facts` | Labs HTTPS on :443; we use HTTP on **81** (`deployment_portal_http_port`) unless `cm_external_url` overrides. |
...
- **Caddy edge port** `deployment_portal_http_port` (default **81**) on ops — one listener, many hostnames (`portal.*`, `cm.*`, `ipa.*`, …). **CM scm-server stays on cldr-mngr :7180 / :7183**; Caddy proxies to those private ports (`deployment_portal_Caddyfile_vhosts.inc.j2`). Browser CM URL: `http://cm.<dashed-ops-ip>.pvc.cloudera-labs.com:81` — not `:7183` on the `cm.*` name.
- Caddy/portal stack runs only when **`monitoring_stack_enabled`** (and `deployment_portal_enabled`) — `deployment_portal_caddy_effective` in `resolve_deployment_portal_caddy_enabled.yml`. With monitoring off, CM install is unchanged (direct API/UI on manager ports).
