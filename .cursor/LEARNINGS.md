...
- **IPA dual UI:** FreeIPA serves **modern** (`/ipa/modern-ui/`) and **legacy** (`/ipa/ui`) on the server FQDN. Caddy vhost `ipa.<dashed-ops-ip>.pvc.cloudera-labs.com:81` uses **`redir / /ipa/modern-ui/ permanent`** (Labs uses `/ipa/ui` for legacy-only); **`reverse_proxy` HTTP** to `<ipaserver-fqdn>` with `header_up Host` and fixed **Referer** (`/ipa/modern-ui/` or `/ipa/ui` per path — cloudera-labs/openshift pattern; no `handle_response` Location rewrite). On **ipaserver**, `zz-ipa-caddy-proxy.conf` (mod_substitute) + relaxed `ipa-rewrite` HTTPS redirect for HTTP from Caddy. Portal index links both UIs (`target=_blank`). Tier A verifies `/`, `/ipa/modern-ui/`, and `/ipa/ui` on the Caddy vhost (`follow_redirects=no` — **301/308 is success**). CM Tier A uses **`probe_cm_manager_ui_http.yml`** on cldr-mngr (manager IP/FQDN `:7180`/`:7183`), not Caddy.
