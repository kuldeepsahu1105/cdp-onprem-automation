...
- **Monitoring:** Grafana/Prometheus/Alertmanager are **not** published on host ports — only **Caddy :8088** (vhost `grafana.<dashed-ip>.pvc.cloudera-labs.com` or path `/grafana/`). Caddy upstreams: Compose service names `grafana` / `prometheus` / `alertmanager` on `deployment_portal`. **cAdvisor** direct **:8089**. PORTAL bootstrap forces `monitoring_stack_enabled=false`; run **32** / Jenkins **MONITORING** to deploy + wire Caddy. Portal index tiles/links use **`deployment_portal_context.monitoring.enabled`** (`monitoring_stack_enabled` **or** detected running `cldr-mon-grafana` / monitoring compose on ops host) — re-run **35_refresh** after MONITORING if the index was rendered with monitoring off. Milestone **`monitoring`** verifies `/grafana/login` and `/prometheus/-/healthy` on localhost.
