...
- **IPA dual UI:** Caddy vhost `ipa.<dashed-ops-ip>.pvc.cloudera-labs.com:81` **`redir / /ipa/modern-ui/`**; **`reverse_proxy` HTTP** to `<ipaserver-fqdn>` with `header_up Host` + **Referer** (cloudera-labs/openshift). **Default:** no ipaserver httpd edits (`deployment_portal_ipa_httpd_proxy_enabled: false`). Opt-in **`zz-ipa-caddy-proxy.conf`** / **`ipa-rewrite`** when that flag is **true** (never **`mod_substitute`** on `/ipa/json`). Caddy sends **`Accept-Encoding: identity`** on `/ipa/json`. FILE ccache documented in **`/etc/krb5.conf.d/ansible-ccache-note.conf`** (`KRB5CCNAME`, not `default_ccache_name`). CM Tier A uses **`probe_cm_manager_ui_http.yml`** on cldr-mngr, not Caddy.
