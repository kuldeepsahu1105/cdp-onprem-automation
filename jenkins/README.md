...
| Parameter | Default | Purpose |
|---|---|---|
| `PIPELINE_STAGES` → **`DESTROY_STACK`** | unchecked | Enables and confirms **Destroy Stack** (`run-destroy-stack.sh` → `terraform destroy` for this **`ENVIRONMENT`** workspace). If the checkbox is missing, run **REFRESH_JENKINSFILE=YES** once. |
| `DRY_RUN` | `false` | With **`DESTROY_STACK`**: `terraform destroy` **plan only** (no apply). |
