#!/usr/bin/env bash
# Jenkins: run Terraform destroy wrapper (destroy plan or apply based on DRY_RUN / DESTROY_STACK_CONFIRM).
set -euo pipefail
...
LOG_FILE="$LOG_DIR/terraform-destroy-${BUILD_NUMBER:-local}.log"
log() { printf '[terraform-destroy] %s\n' "$*" | tee -a "$LOG_FILE"; }
...
log "Starting clone_and_run_terraform_destroy.sh (DRY_RUN=${DRY_RUN}, DESTROY_STACK_CONFIRM=${DESTROY_STACK_CONFIRM})"
# shellcheck source=scripts/lib/jenkins_log_pipe.sh
source "$REPO_ROOT/scripts/lib/jenkins_log_pipe.sh"
set -o pipefail
jenkins_log_pipe "$LOG_FILE" ./clone_and_run_terraform_destroy.sh
log "Terraform destroy stage completed"
