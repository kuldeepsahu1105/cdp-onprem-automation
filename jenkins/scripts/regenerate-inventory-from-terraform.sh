#!/usr/bin/env bash
# Regenerate inventory for an external Jenkins controller.
# SSH reaches the public IPAServer first, then private cluster addresses via ProxyJump.
set -euo pipefail
...
INVENTORY_SSH_MODE="${INVENTORY_SSH_MODE:-bastion}" bash "$GEN_SCRIPT"
...
log "Wrote $DEST_INVENTORY (SSH mode: ${INVENTORY_SSH_MODE:-bastion})"
