...

{
  ...
  http_port="$(read_group_var deployment_portal_http_port 8088)"
  ...
  if [[ "$private_profile" == "true" ]]; then
    ...
    if [[ "$monitoring_on" == "true" || "$monitoring_on" == "1" ]]; then
      echo "  Grafana:    $(url_with_port http "$ops_fqdn" "$http_port" /grafana/)"
      echo "  Prometheus: $(url_with_port http "$ops_fqdn" "$http_port" /prometheus/)"
    fi
  else
    ...
    if [[ "$monitoring_on" == "true" || "$monitoring_on" == "1" ]]; then
      echo "  Grafana:    $(url_with_port http "$ops_pub" "$http_port" /grafana/)"
      echo "  Prometheus: $(url_with_port http "$ops_pub" "$http_port" /prometheus/)"
    fi
    echo "VPC private IP:"
    echo "  Portal:     $(url_with_port http "$ops_priv" "$http_port" /)"
    echo "  pgAdmin:    $(url_with_port http "$ops_priv" "$pg_port" /)"
    if [[ "$monitoring_on" == "true" || "$monitoring_on" == "1" ]]; then
      echo "  Grafana:    $(url_with_port http "$ops_priv" "$http_port" /grafana/)"
      echo "  Prometheus: $(url_with_port http "$ops_priv" "$http_port" /prometheus/)"
    fi
  fi
  echo ""
  ...
  echo ""
  echo "Service URL verify tiers (RUNBOOK.md): Tier A = localhost on service host (portal 127.0.0.1:8088, CM 127.0.0.1:7180 on cldr-mngr);"
